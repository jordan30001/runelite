...
public final class Constants {
	...
	public static final class TwitchChat {
		...
		private static final String initUserCommands() {
			return Arrays.stream(TwitchChat.class.getDeclaredFields()).filter(field -> field.getName().startsWith("USER_") && Modifier.isStatic(field.getModifiers())).map(Utils.ThrowingFunctionWrapper(field -> field.get(null).toString()))
					.collect(Collectors.joining("|"));
		}

		private static final String initModCommands() {
			return Arrays.stream(TwitchChat.class.getDeclaredFields()).filter(field -> field.getName().startsWith("MOD_") && Modifier.isStatic(field.getModifiers())).map(Utils.ThrowingFunctionWrapper(field -> field.get(null).toString()))
					.collect(Collectors.joining("|"));
		}
	}
}